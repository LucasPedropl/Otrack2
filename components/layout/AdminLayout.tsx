		...
		items: [
			{
				...
				permission: 'dashboard:view', // Usando dashboard view como acesso básico ou acesso_perfis
			},
		],
	},
	{
		...
		items: [
			{
				...
				icon: Users,
				permission: 'mao_obra_colaboradores:view',
			},
		],
	},
	{
...
const AdminLayoutContent: React.FC<LayoutProps> = ({ children }) => {
	...
	const { sites: allSitesList, isLoading: isSitesLoading } =
	...
	const [hoveredSettingsMenu, setHoveredSettingsMenu] = useState<
		string | null
	>(null);
	const [hoveredMenuPosition, setHoveredMenuPosition] = useState<{
		top: number;
		right: number;
	} | null>(null);
	...
	useEffect(() => {
		...
		const shouldAutoOpen = SETTINGS_PATHS.some((path) =>
	...
	const isSettingsPath = SETTINGS_PATHS.some((path) =>
		location.pathname.startsWith(path),
	);
	...
	return (
		...
		>
			...
			<aside
				className={`fixed md:relative inset-y-0 left-0 z-50 flex flex-col flex-shrink-0 transition-all duration-300 ease-in-out shadow-2xl md:shadow-none ${isMobileOpen ? 'translate-x-0 w-64' : '-translate-x-full md:translate-x-0'} ${isPrimaryCollapsed ? 'md:w-20' : 'md:w-64'}`}
				style={{
			...
			>
				...
				<div
					className={`h-[81px] p-6 flex items-center gap-3 transition-all ${isPrimaryCollapsed ? 'md:justify-center' : ''} relative`}
				>
					<div
					...
					/>
					<div
						className={`overflow-hidden whitespace-nowrap ${isPrimaryCollapsed && !isMobileOpen ? 'md:hidden' : 'block'}`}
					>
				...
				<nav className="p-4 space-y-2 flex-1 overflow-y-auto overflow-x-hidden no-scrollbar">
					{navItems.map((item) => {
						...
						return (
							<button
								...
								onMouseEnter={(e) =>
									handleTooltip(e, item.label, 'primary')
								}
								onMouseLeave={handleTooltipLeave}
								className={`group relative w-full flex items-center gap-3 ${isPrimaryCollapsed ? 'md:justify-center' : ''} px-4 py-3 rounded-lg transition-all hover:bg-white/5`}
								style={getSidebarItemStyle(isActive)}
							>
								<item.icon className="h-5 w-5 flex-shrink-0" />
								<span
									className={`whitespace-nowrap ${isPrimaryCollapsed && !isMobileOpen ? 'md:hidden' : 'block'}`}
								>
									{item.label}
								</span>
							</button>
					...
					{(hasAnyObraPermission || isAdmin) && (
						<div className="pt-4 mt-2">
							<p
								className={`px-4 text-xs font-bold uppercase tracking-wider mb-2 opacity-50 ${isPrimaryCollapsed && !isMobileOpen ? 'md:hidden' : 'block'}`}
							>
								Obras Ativas
							</p>
							<div className="space-y-1">
								{sidebarSites.map((site) => (
									<button
										...
										onMouseLeave={handleTooltipLeave}
										className={`group relative w-full flex items-center gap-3 ${isPrimaryCollapsed ? 'md:justify-center' : ''} px-4 py-2 rounded-lg transition-all hover:bg-white/5`}
									...
									>
										...
										<span
											className={`whitespace-nowrap text-sm truncate ${isPrimaryCollapsed && !isMobileOpen ? 'md:hidden' : 'block'}`}
										>
											{site.name}
										</span>
									</button>
				...
				{true && (
					<div className="p-4 relative">
						...
						<button
							...
							onMouseEnter={(e) =>
								handleTooltip(e, 'Configurações', 'primary')
							}
							onMouseLeave={handleTooltipLeave}
							className={`group relative w-full flex items-center gap-3 ${isPrimaryCollapsed ? 'md:justify-center' : ''} px-4 py-3 rounded-lg transition-all hover:bg-white/5`}
							style={getSidebarItemStyle(
			...
			<div className="flex-1 flex flex-col h-full overflow-hidden w-full">
				...
				<div className="flex-1 flex overflow-hidden relative">
					<main
						className={`flex-1 relative w-full h-full overflow-y-auto flex flex-col flex-1 ${!location.pathname.startsWith('/admin/obra/') && !location.pathname.startsWith('/admin/dashboard') ? 'p-4 sm:p-8' : ''}`}
					>
						{children}
						{/* Spacer for mobile bottom bar */}
						{isSettingsPath && window.innerWidth < 768 && (
							<div className="h-24 flex-shrink-0 md:hidden" />
						)}
					</main>

					{showToggleStrip && (
						<div
							className="hidden md:flex items-center justify-center cursor-pointer group z-40 relative flex-shrink-0"
							style={{
								width: '16px',
							}}
						...
						>
							...
							{/* Visible button */}
							<div
								className="relative w-5 h-8 flex items-center justify-center rounded-l-md shadow-sm transition-transform duration-200 hover:scale-110 active:scale-95 border-r-0"
								style={{
									backgroundColor:
										currentTheme.colors.sidebar, // Match sidebar color
									color: currentTheme.colors.sidebarText,
									position: 'absolute',
									right: '-1px', // Sit exactly on the border line
								}}
							>
								{isSettingsCollapsed ? (
									<ChevronLeft size={12} />
					...
					<aside
						className={`hidden md:flex flex-shrink-0 transition-all duration-300 flex-col z-20 relative border-l border-solid ${isSettingsOpen ? (isSettingsCollapsed ? 'md:w-20 overflow-visible' : 'w-full md:w-64 overflow-y-auto') : 'w-0 overflow-hidden'}`}
						style={{
							backgroundColor: currentTheme.colors.sidebar,
							color: currentTheme.colors.sidebarText,
						...
						<div className="p-4 space-y-6">
							{settingsMenus.map((menu) => (
								...
								>
									...
									>
										<div
											className={`flex items-center gap-2 font-semibold ${isSettingsCollapsed ? 'justify-center' : ''}`}
											style={{
									...
									{(!isSettingsCollapsed ||
										...
										openMenus[menu.id] && (
											<div
												className="ml-4 pl-4 border-l border-solid space-y-1"
												style={{
											...
											>
												{menu.items.map((subItem) => (
													<button
														...
														className="block w-full text-left py-2 px-3 rounded text-sm transition-all hover:bg-black/5 dark:hover:bg-white/10 hover:!opacity-100"
														style={getSidebarItemStyle(
															location.pathname ===
																subItem.path,
														)}
				...
				hoveredMenuPosition && (
					<div
						...
						style={{
							...
							right: hoveredMenuPosition.right,
							backgroundColor: currentTheme.colors.card,
							borderColor: currentTheme.colors.border,
						}}
					...
					>
						...
						>
							{
								settingsMenus.find(
									(m) => m.id === hoveredSettingsMenu,
								)?.label
							}
						</div>
						<div className="p-2 space-y-1">
							{settingsMenus
								.find((m) => m.id === hoveredSettingsMenu)
								?.items.map((subItem) => (
									<button
										key={subItem.path}
										onClick={() =>
			...
			{isSettingsPath && (
				<>
					<div
						className="md:hidden fixed bottom-6 left-6 right-6 h-16 rounded-2xl border shadow-lg z-50 flex items-center justify-around pb-0 animate-in slide-in-from-bottom-5 duration-300"
						style={{
							backgroundColor: currentTheme.colors.card, // Using card instead of background for float effect
							borderColor: currentTheme.colors.border,
						}}
					>
						<button
							onClick={() =>
								handleSettingsNavigate('/admin/dados')
							}
							className={`flex flex-col items-center gap-1 p-2 rounded-lg transition-colors ${
								location.pathname === '/admin/dados'
									? 'opacity-100'
									: 'opacity-50'
							}`}
							style={{
						...
						<button
							onClick={() =>
								handleSettingsNavigate('/admin/usuarios')
							}
							className={`flex flex-col items-center gap-1 p-2 rounded-lg transition-colors ${
						...
						<button
							...
							className={`flex flex-col items-center gap-1 p-2 rounded-lg transition-colors ${isMobileMenuOpen ? 'opacity-100' : 'opacity-50'}`}
							style={{
					...
					{/* Mobile Menu Overlay */}
					{isMobileMenuOpen && (
						<div className="fixed inset-0 z-[60] md:hidden flex flex-col animate-in slide-in-from-bottom-10 fade-in duration-200">
							<div
							...
							<div
								className="absolute inset-x-4 bottom-24 rounded-2xl flex flex-col p-6 z-10 overflow-y-auto max-h-[80vh] shadow-2xl"
								style={{
									backgroundColor: currentTheme.colors.card,
								}}
							>
								...
								<div className="space-y-6 pb-4">
									...
									<div className="grid grid-cols-2 gap-3">
										<button
											...
											className="p-3 rounded-xl border flex flex-col items-center gap-2 active:scale-95 transition-transform"
											style={{
												backgroundColor:
													currentTheme.colors
														.background,
									...
									{settingsMenus.map((menu) => (
										<div key={menu.id}>
											<h3
												className="text-xs font-bold uppercase tracking-wider mb-2 opacity-50"
												style={{
													color: currentTheme.colors
														.text,
												}}
											...
											<div className="space-y-1">
												{menu.items.map((item) => (
													<button
														...
														className="w-full flex items-center gap-3 p-3 rounded-lg active:bg-black/5 transition-colors"
														style={{
															color:
																location.pathname ===
																item.path
