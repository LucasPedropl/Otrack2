...
const UnidadesPage: React.FC = () => {
	...
	const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		try {
			...
			for (const row of data) {
				const name =
					row['Nome'] || row['NOME'] || row['nome'] || row['Unidade'];
				const abbr =
					row['Abreviação'] ||
					row['Sigla'] ||
					row['ABREVIACAO'] ||
			...
			alert(
				"Erro ao importar arquivo. Verifique se é um Excel/CSV válido com colunas 'Nome' e 'Abreviação'.",
			);
		} finally {
			setIsLoading(false);
		}
	...
	const confirmBulkDelete = async () => {
		...
		try {
			await Promise.all(
				idsToDelete.map((id) => settingsService.deleteUnit(id)),
			);
		...
		<>
								...
								currentData.map((unit, index) => {
									...
									return (
										...
										>
											...
											<td
												className="p-4 font-mono font-bold whitespace-nowrap"
												style={{
													color: currentTheme.colors
														.primary,
												}}
			...
			{isModalOpen && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
					<div
						className="w-full max-w-md rounded-2xl shadow-xl border p-6 mx-4"
						style={{
							backgroundColor: currentTheme.colors.card,
							borderColor: currentTheme.colors.border,
						}}
			...
			{deleteId && (
				...
				>
					...
					>
						...
						>
							Tem certeza que deseja excluir esta unidade? Esta
