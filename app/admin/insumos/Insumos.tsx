...
const InsumosPage: React.FC = () => {
	...
	const fetchData = async () => {
		try {
			const [inventoryData, unitsData, categoriesData] =
				await Promise.all([
	...
	useEffect(() => {
		const handleClickOutside = (event: MouseEvent) => {
			...
			if (
				costTypeRef.current &&
				!costTypeRef.current.contains(event.target as Node)
			) {
				setShowCostTypeOptions(false);
			}
			if (
				unitRef.current &&
				!unitRef.current.contains(event.target as Node)
			) {
	...
	const handleQuickAddCategory = async () => {
		...
		const exists = availableCategories.find(
			(c) => c.category.toLowerCase() === newCategoryName.toLowerCase(),
		);
		if (exists) {
		...
		if (
			window.confirm(
				`Deseja cadastrar a nova categoria "${newCategoryName}"?`,
			)
		) {
			try {
	...
	const handleQuickAddUnit = async () => {
		...
		// Check if exists
		const exists = availableUnits.find(
			(u) =>
				u.name.toLowerCase() === newUnitName.toLowerCase() ||
				u.abbreviation.toLowerCase() === newUnitName.toLowerCase(),
		);
		if (exists) {
		...
		const abbr = prompt(
			`Informe a abreviação para "${newUnitName}" (ex: UN, kg, m):`,
		);
		if (abbr) {
			try {
	...
	const handleExport = () => {
		...
		const csvContent =
			...
			items
				.map(
					(i) =>
						`${i.code || ''},${i.name},${i.category},${i.costType},${i.unit},${i.quantity},${i.unitValue},${i.minThreshold}`,
				)
				.join('\n');
	...
	const handleImport = async () => {
		alert(
			'Funcionalidade de importação de insumos em massa via Excel será implementada em breve.',
		);
	};
	...
	const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
		if (e.target.checked) {
			const allIdsOnPage = currentData
				.map((item) => item.id!)
				.filter((id) => !!id);
			const newSelection = new Set(selectedIds);
	...
	const confirmBulkDelete = async () => {
		...
		try {
			await Promise.all(
				idsToDelete.map((id) => inventoryService.delete(id)),
			);
	...
	const isAllSelected =
		...
		currentData.every((item) => item.id && selectedIds.has(item.id));
	...
	const showQuickAddCategory =
		...
		!filteredCategories.some(
			(c) =>
				c.category.toLowerCase() === formData.category?.toLowerCase(),
		);

	// Determine if we need to show quick add for Unit
	const showQuickAddUnit =
		formData.unit &&
		!filteredUnits.some(
			(u) =>
				u.name.toLowerCase() === formData.unit?.toLowerCase() ||
				u.abbreviation.toLowerCase() === formData.unit?.toLowerCase(),
		);
							...
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
								{currentData.map((item) => (
									...
									>
										...
										<div>
											<h3
												className="font-bold text-lg mb-1 pr-6"
												style={{
													color: currentTheme.colors
														.text,
												}}
											...
											<div
												className="grid grid-cols-2 gap-2 text-sm mt-4 pt-4 border-t"
												style={{
													borderColor:
														currentTheme.colors
															.border,
				...
				>
					...
					>
						...
						<p
							...
							style={{ color: currentTheme.colors.textSecondary }}
						>
							Tem certeza que deseja excluir este insumo? Esta
