...
const CategoriasPage: React.FC = () => {
	...
	const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (file.type === 'application/pdf') {
			alert(
				'A importação de PDF requer um processamento de OCR avançado. Por favor, utilize um arquivo Excel (.xlsx) ou CSV para uma importação precisa e imediata.',
			);
			return;
		}
		...
		try {
			...
			alert(
				'Erro ao importar arquivo. Verifique se é um Excel/CSV válido.',
			);
		} finally {
			setIsLoading(false);
		}
	...
	const handleExport = () => {
		...
		const csvContent =
			...
			categories
				.map(
					(c) =>
						`${c.type},${c.category},${c.subcategory},${c.registrationType}`,
				)
				.join('\n');
	...
	const confirmBulkDelete = async () => {
		...
		try {
			await Promise.all(
				idsToDelete.map((id) => settingsService.deleteCategory(id)),
			);
	...
	return (
		<>
			...
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
				...
				{hasPermission('orcamento_categorias', 'create') && (
					<Button
						onClick={() => {
							setFormData({
								type: 'Produto',
								...
								currentData.map((item, index) => {
									...
									return (
										...
										>
											...
											<td className="p-4">
												<span
													className={`px-2 py-1 rounded text-xs font-medium ${item.type === 'Produto' ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'}`}
												>
													{item.type}
												</span>
											</td>
			...
			<BottomActionsBar
				...
				onDeleteSelected={
					hasPermission('orcamento_categorias', 'delete')
			...
			{isModalOpen && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
					<div
						className="w-full max-w-lg rounded-2xl shadow-xl border p-6 mx-4"
						style={{
							backgroundColor: currentTheme.colors.card,
							borderColor: currentTheme.colors.border,
						}}
			...
			{deleteId && (
				...
				>
					...
					>
						...
						>
							Tem certeza que deseja excluir esta categoria? Esta
