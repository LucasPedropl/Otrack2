...
const UsuariosPage: React.FC = () => {
	...
	const handleExport = () => {
		...
		const csvContent =
			...
			users
				.map((u) => {
					const profileName =
						profiles.find((p) => p.id === u.profileId)?.name ||
						u.role;
	...
	const confirmBulkDelete = async () => {
		...
		try {
			await Promise.all(
				safeToDeleteIds.map((id) => userService.delete(id)),
			);
		...
		<>
								...
								currentData.map((item, index) => {
									...
									return (
										...
										>
											...
											<td className="p-4">
												<span
													className="flex items-center gap-2 px-2 py-1 rounded-full text-xs font-bold w-fit bg-opacity-10 border opacity-80"
													style={{
														backgroundColor:
															'rgba(99, 102, 241, 0.1)',
														color: currentTheme
			...
			{isModalOpen && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
					<div
						className="w-full max-w-lg rounded-2xl shadow-xl border p-6 mx-4 max-h-[90vh] overflow-y-auto"
						style={{
							backgroundColor: currentTheme.colors.card,
							borderColor: currentTheme.colors.border,
						}}
				...
				>
					...
					>
						...
						>
							...
							<b>{selectedIds.size}</b> usuários selecionados?
							<br />
