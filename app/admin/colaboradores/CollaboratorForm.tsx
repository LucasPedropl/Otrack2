...
const CollaboratorForm: React.FC<CollaboratorFormProps> = ({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (photoFile) {
				fotoUrl = await collaboratorService.uploadFile(
					photoFile,
					`collaborators/${Date.now()}_${photoFile.name}`,
				);
			}

			const newAttachments = [];
			for (const file of attachmentFiles) {
				const url = await collaboratorService.uploadFile(
					file,
				...
				newAttachments.push({
					name: file.name,
					url,
					path: `attachments/${Date.now()}_${file.name}`,
				});
			}
	...
	return (
		...
		>
			...
			>
							...
							<div className="flex flex-col md:flex-row gap-6">
								<div className="flex-1 space-y-4">
									<div>
										...
										>
											<option value="Geplano Gestão e Consultoria de Obras LTDA">
												Geplano Gestão e Consultoria de
												Obras LTDA
											</option>
				...
				<div
					className="p-6 border-t flex justify-end gap-3 rounded-b-xl"
					style={{
						borderColor: currentTheme.colors.border,
						backgroundColor: currentTheme.isDark
							? 'rgba(0,0,0,0.2)'
