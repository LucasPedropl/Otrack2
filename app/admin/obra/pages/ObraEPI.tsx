...
import {
	siteCollaboratorService,
	SiteCollaborator,
} from '../../../../services/siteCollaboratorService';
...
const ObraEPI: React.FC = () => {
	...
	const [siteCollaborators, setSiteCollaborators] = useState<
		SiteCollaborator[]
	>([]);
	const [globalCollaborators, setGlobalCollaborators] = useState<
		Collaborator[]
	>([]);
	...
	const [selectedCollaboratorHistory, setSelectedCollaboratorHistory] =
		useState<EPIWithdrawal[]>([]);
	...
	const [withdrawalCollaboratorSearch, setWithdrawalCollaboratorSearch] =
		useState('');
	...
	const fetchData = async (showLoader = false) => {
		...
		try {
			...
			try {
				siteCollabs =
					await siteCollaboratorService.getSiteCollaborators(siteId);
			} catch (e) {
	...
	const handleSubmitWithdrawal = async (e: React.FormEvent) => {
		...
		try {
			...
			const siteCollab = siteCollaborators.find(
			...
			if (siteCollab) {
				...
				const globalCollab = globalCollaborators.find(
					(c) => c.id === selectedCollaboratorId,
				);
				if (globalCollab) {
					...
					console.warn(
						'Collaborator details not found for ID:',
						selectedCollaboratorId,
					);
	...
	const handleAddCollaborator = async (globalCollabId: string) => {
		...
		try {
			const globalCollab = globalCollaborators.find(
				(c) => c.id === globalCollabId,
			);
			if (!globalCollab)
				throw new Error('Colaborador global não encontrado');

			await siteCollaboratorService.addCollaboratorToSite(
				siteId,
				globalCollab,
			);
			...
			alert(
				'Erro ao adicionar colaborador. Verifique se já não está cadastrado.',
			);
		} finally {
			setSubmitting(false);
		}
	...
	const handleRemoveCollaborator = async (collabId: string) => {
		if (
			!siteId ||
			!window.confirm(
				'Tem certeza que deseja remover este colaborador da obra?',
			)
		)
			return;
		try {
			await siteCollaboratorService.removeCollaboratorFromSite(
				siteId,
				collabId,
			);
			fetchData();
	...
	const handleViewHistory = (collabName: string) => {
		const history = withdrawals.filter(
			(w) => w.collaboratorName === collabName,
		);
	...
	const filteredHistory = withdrawals.filter(
		(w) =>
			w.collaboratorName
				.toLowerCase()
	...
	const filteredGlobalCollaborators = globalCollaborators
		.filter(
			(gc) =>
				!siteCollaborators.some(
					(sc) => sc.originalCollaboratorId === gc.id,
				),
		)
		.filter((gc) =>
			gc.nome.toLowerCase().includes(addCollaboratorSearch.toLowerCase()),
		);

	const filteredWithdrawalCollaborators = siteCollaborators.filter((c) =>
		c.name
			.toLowerCase()
			.includes(withdrawalCollaboratorSearch.toLowerCase()),
	);

	const baseInputClass =
		'w-full px-3 py-2 border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition-colors';
	...
	return (
		<div className="space-y-6">
				...
				(viewMode === 'card' ? (
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{filteredEPIs.map((item) => (
							...
							>
								<div>
									...
									<p
										className={`font-bold text-sm mb-4 ${item.quantity === 0 ? 'text-red-500' : 'text-green-500'}`}
									>
						...
						{filteredEPIs.length === 0 && (
							<p className="col-span-full text-center py-8 opacity-50">
							...
							>
								{filteredEPIs.map((item) => (
									...
									>
										...
										<td className="p-4">
											<span
												className={`font-bold ${item.quantity === 0 ? 'text-red-500' : 'text-green-500'}`}
											>
												{item.quantity}
											</span>
										</td>
							...
							{filteredCollaborators.length === 0 && (
								<tr>
									<td
										...
										className="p-8 text-center opacity-50"
									>
										Nenhum colaborador cadastrado nesta
										obra.
									</td>
								</tr>
							)}
						...
						>
							{filteredHistory.map((h) => (
								<tr
									...
									onClick={() =>
										handleViewHistory(h.collaboratorName)
									}
									style={{
										backgroundColor:
											currentTheme.colors.card,
						...
						>
							<div>
								...
								<div className="relative">
									<div
										className="flex items-center border rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-brand-500"
										style={{
											borderColor:
									...
									{/* List of matches */}
									<div
										className="mt-1 max-h-40 overflow-y-auto border rounded-lg"
										style={{
											borderColor:
												currentTheme.colors.border,
											backgroundColor:
									...
									>
										{filteredWithdrawalCollaborators.length >
										0 ? (
											filteredWithdrawalCollaborators.map(
												(c) => (
													<div
														key={c.id}
														onClick={() => {
											...
											<div
												className="p-2 text-sm opacity-50 text-center"
												style={{
													color: currentTheme.colors
														.text,
												}}
											>
												{withdrawalCollaboratorSearch
								...
								{siteCollaborators.length === 0 && (
									<p className="text-xs text-red-500 mt-1">
										Nenhum colaborador cadastrado na obra.
									</p>
								)}
							</div>
							...
							<div className="relative" ref={itemRef}>
								...
								<div className="relative">
									...
									/>
									<Search
										className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 opacity-40"
										style={{
											color: currentTheme.colors.text,
										}}
									/>
								...
								{showItemOptions && (
									<ul
										className="absolute z-50 w-full mt-1 max-h-48 overflow-auto rounded-lg border shadow-xl"
										style={{
											backgroundColor:
												currentTheme.colors.card,
											borderColor:
										...
										{epiItems
											.filter((item) =>
												item.name
													.toLowerCase()
													.includes(
														withdrawalItemSearch.toLowerCase(),
													),
											)
											.map((item) => (
												<li
							...
							<div className="flex justify-end gap-3 pt-4">
								...
								<Button
									type="submit"
									disabled={
										submitting || !selectedCollaboratorId
									}
					...
					>
						...
						<div className="space-y-4">
							<div>
								...
								>
									{filteredGlobalCollaborators.length > 0 ? (
										filteredGlobalCollaborators.map((c) => (
											<div
												key={c.id}
												className="p-3 flex justify-between items-center hover:bg-opacity-50"
												style={{
													backgroundColor:
														currentTheme.colors
															.background,
								...
								>
									Apenas colaboradores não cadastrados nesta
									obra aparecem aqui.
								</p>
							</div>
			...
			{showHistoryModal && (
				...
				>
					<div
						className="w-full max-w-2xl rounded-xl shadow-2xl border flex flex-col max-h-[85vh]"
						style={{
							backgroundColor: currentTheme.colors.card,
							borderColor: currentTheme.colors.border,
						}}
					...
					>
						...
						>
							<h3
								...
								style={{ color: currentTheme.colors.text }}
							>
								Histórico: {selectedCollaboratorName}
							</h3>
						...
						<div className="flex-1 overflow-y-auto p-4">
							{selectedCollaboratorHistory.length === 0 ? (
								<p className="text-center opacity-50 py-8">
									Nenhuma retirada registrada para este
									colaborador.
								</p>
							) : (
								<table className="w-full text-left text-sm">
									...
									>
										{selectedCollaboratorHistory.map(
											(h) => (
												<tr key={h.id}>
													<td
														className="p-3"
