...
const ObraTools: React.FC = () => {
	...
	const [activeTab, setActiveTab] = useState<
		'inventory' | 'active_loans' | 'history'
	>('inventory');
	...
	const [selectedTool, setSelectedTool] = useState<
		SiteInventoryItem | RentedEquipment | null
	>(null);
	...
	const [selectedLoanToReturn, setSelectedLoanToReturn] =
		useState<ToolLoan | null>(null);
	...
	const fetchData = async () => {
		...
		try {
			...
			setRentedEquipment(
				activeRented.filter((r) => r.status === 'ACTIVE'),
			);
	...
	const getAvailableQuantity = (
		item: SiteInventoryItem | RentedEquipment,
	) => {
	...
	const handleSubmitLoan = async (e: React.FormEvent) => {
		e.preventDefault();
		if (!siteId || !selectedTool || isSubmitting) return;
		...
		const combinedDate = new Date(
			`${loanData.loanDate}T${loanData.loanTime}`,
		);
	...
	const handleSubmitReturn = async (e: React.FormEvent) => {
		e.preventDefault();
		if (!siteId || !selectedLoanToReturn || isSubmitting) return;

		setIsSubmitting(true);
		// Combine Date and Time
		const combinedDate = new Date(
			`${returnData.returnDate}T${returnData.returnTime}`,
		);
	...
	const toggleItemAsTool = async (
		...
		if (!newStatus) {
			...
			if (hasLoans) {
				alert(
					'Não é possível remover da lista de ferramentas pois existem empréstimos ativos para este item. Devolva-os primeiro.',
				);
				return;
			}
		}
		...
		try {
			if (isRented) {
				setRentedEquipment((prev) =>
					prev.map((i) =>
						i.id === item.id ? { ...i, isTool: newStatus } : i,
					),
				);
				await rentedEquipmentService.update(siteId, item.id!, {
					isTool: newStatus,
				});
			} else {
				setInventory((prev) =>
				...
				await siteInventoryService.updateItem(siteId, item.id!, {
					isTool: newStatus,
				});
			}
		} catch (error) {
	...
	const unifiedTools: UnifiedToolDisplay[] = [
		...
		const matchesSearch =
			...
			(item.supplier?.toLowerCase() || '').includes(
	...
	return (
		<div className="space-y-6">
					...
					<div>
						...
						>
							{inventory
								.filter((i) => isTool(i))
								.reduce(
									(acc, i) => acc + getAvailableQuantity(i),
									0,
								)}
						</p>
					</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
							{unifiedTools.map((tool) => (
								<div
									...
									className="p-4 rounded-xl border flex flex-col justify-between relative overflow-hidden group"
									style={{
										backgroundColor:
							...
							{unifiedTools.length === 0 && (
								<p
									className="col-span-full text-center py-8 opacity-50"
									style={{ color: currentTheme.colors.text }}
								>
								...
								>
									{unifiedTools.map((tool) => (
										...
										>
											...
											<td className="p-4">
												<span
													className={`px-2 py-1 rounded text-xs font-bold ${tool.type === 'RENTED' ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-blue-600'}`}
												>
													{tool.type === 'RENTED'
														? 'Alugado'
														: 'Próprio'}
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							{activeLoans.map((loan) => {
								...
								return (
									<div
										key={loan.id}
										className={`p-4 rounded-xl border flex justify-between items-center ${isOverdue ? 'border-red-500/30 bg-red-500/5' : ''}`}
										style={{
									...
									>
										<div>
											<div className="flex items-center gap-2">
												...
												{isRented && (
													<span className="text-[10px] bg-purple-100 text-purple-600 px-1.5 rounded font-bold">
														ALUGADO
													</span>
												)}
												{isOverdue && (
											...
											>
												Retirado em{' '}
												{loan.loanDate.toLocaleDateString()}{' '}
												às{' '}
												{loan.loanDate.toLocaleTimeString(
													[],
													{
														hour: '2-digit',
														minute: '2-digit',
										...
										<Button
											...
											variant="secondary"
											className="h-10 border-orange-200 hover:bg-orange-50 text-orange-600"
										>
											<RotateCcw
												size={16}
												className="mr-2"
							...
							{loanHistory.map((h) => (
								...
								>
									...
									>
										...
										{h.itemOrigin === 'RENTED' && (
											<span className="ml-2 text-[10px] bg-purple-100 text-purple-600 px-1 rounded">
												Locado
											</span>
										)}
									</td>
									...
									<td className="p-3">
										<span
											className={`px-2 py-0.5 rounded text-xs ${h.status === 'OPEN' ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'}`}
										>
											{h.status === 'OPEN'
												? 'Em uso'
												: 'Devolvido'}
			...
			{isLoanModalOpen && selectedTool && (
				...
				>
					<div
						className="w-full max-w-sm rounded-2xl shadow-xl border p-6"
						style={{
							backgroundColor: currentTheme.colors.card,
							borderColor: currentTheme.colors.border,
						}}
					...
					>
						...
						<div
							className="mb-4 p-3 rounded bg-opacity-5 border"
							style={{
								backgroundColor:
						...
						>
							...
							>
								Disponível: {getAvailableQuantity(selectedTool)}{' '}
								{selectedTool.unit}
							</p>
							{'supplier' in selectedTool && (
				...
				>
					...
					>
						...
						<div className="mb-4 p-3 rounded bg-orange-500/10 border border-orange-500/20">
							...
							>
								Responsável: {selectedLoanToReturn.workerName}
							</p>
							<p
								className="text-sm"
								style={{
							...
							>
								Qtd: {selectedLoanToReturn.quantity}
							</p>
						</div>
			...
			{isManageListOpen && (
				<div
					className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
					onClick={() => setIsManageListOpen(false)}
				>
					<div
						className="w-full max-w-lg rounded-2xl shadow-xl border flex flex-col max-h-[85vh]"
						style={{
							backgroundColor: currentTheme.colors.card,
							borderColor: currentTheme.colors.border,
						}}
					...
					>
						...
						>
							...
							<button onClick={() => setIsManageListOpen(false)}>
								<X
									size={20}
									style={{
										color: currentTheme.colors
								...
								.map((item) => {
									...
									return (
										<div
											...
											className="flex items-center justify-between p-3 hover:bg-white/5 rounded cursor-pointer"
											onClick={() =>
												toggleItemAsTool(item)
											}
										>
											<div>
												<p
											...
											<div
												className={`transition-colors ${isActive ? 'text-blue-500' : 'text-gray-400'}`}
											>
												{isActive ? (
													<CheckSquare size={24} />
												) : (
