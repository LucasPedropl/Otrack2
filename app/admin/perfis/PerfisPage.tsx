...
const MODULE_GROUPS: ModuleGroup[] = [
	{
		...
		modules: [
			...
			},
			{
				id: 'obra_movements',
				label: 'Movimentações',
				actions: [{ id: 'view', label: 'Visualizar' }],
			},
		],
	},
	{
		...
		modules: [
			// Orçamento
			{
				id: 'orcamento_insumos',
				label: 'Insumos',
				actions: COMMON_ACTIONS,
				category: 'Orçamento',
			},
			{
				id: 'orcamento_unidades',
				label: 'Unidades de Medida',
				actions: COMMON_ACTIONS,
				category: 'Orçamento',
			},
			{
				id: 'orcamento_categorias',
				label: 'Categorias',
				actions: COMMON_ACTIONS,
				category: 'Orçamento',
			},
...
const PerfisPage: React.FC = () => {
	...
	const hasModuleAccess = (module: string) => {
		const prefix = `${module}:`;
		return (
			Array.from(permissions).some((p: string) => p.startsWith(prefix)) ||
	...
	const isFullAccess = (module: string, actions: ModuleAction[]) => {
		...
		return actions.every((action) =>
			permissions.has(`${module}:${action.id}`),
		);
	};
	...
	const handleEdit = (profile: AccessProfile) => {
		...
		setAccessAllSites(
			profile.allSites ?? profile.allowedSites?.length === 0,
		);
		setAllowedSites(new Set(profile.allowedSites || []));
		setEditingId(profile.id!);
		setIsModalOpen(true);
		setExpandedModules(
			new Set(MODULE_GROUPS.flatMap((g) => g.modules.map((m) => m.id))),
		);
	};
	...
	const handleDeleteEverything = async () => {
		...
		if (
			!window.confirm(
				'Isso excluirá os usuários e o perfil. Deseja continuar?',
			)
		)
			return;
		setIsLoading(true);
		try {
			await Promise.all(
				affectedUsers.map((u) => userService.delete(u.id!)),
			);
			await accessProfileService.delete(deleteId);
			handleCloseDelete();
			fetchProfiles();
	...
	const renderModule = (module: SystemModule) => {
		...
		return (
			...
			>
				<div
					className="flex items-center justify-between p-3 bg-opacity-50 hover:bg-opacity-100 transition-colors"
					style={{
				...
				{isExpanded && (
					<div
						className="p-4 pl-10 border-t bg-opacity-30 bg-black/5"
		...
		<>
					...
					<table className="w-full text-left text-sm border-collapse min-w-[500px]">
						<thead>
							...
							>
								...
								<th
									className="p-4 font-medium w-24 text-center"
									style={{
										color: currentTheme.colors
											.textSecondary,
									}}
							...
							{currentData.length === 0 ? (
								...
								currentData.map((item, index) => (
									...
									>
										...
										<td className="p-4">
											<span
												className="text-xs px-2 py-1 rounded bg-opacity-10 border opacity-80"
												style={{
													backgroundColor:
														'rgba(99, 102, 241, 0.1)',
													color: currentTheme.colors
					...
					>
						...
						>
							<div>
								...
								>
									Defina as permissões para este grupo de
									usuários.
								</p>
							</div>
							<button
										...
										return (
											<div key={group.id}>
												<div className="flex items-center justify-between mb-3">
													<div
														className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider opacity-70"
														style={{
															color: currentTheme
																.colors.primary,
														}}
												...
												<div className="space-y-6">
													...
													{uncategorizedModules.length >
														0 && (
														<div className="space-y-3">
															{uncategorizedModules.map(
																(module) =>
																	renderModule(
																		module,
																	),
						...
						<div
							className="p-4 border-t flex justify-end gap-3 bg-opacity-50 bg-gray-50/5"
							style={{ borderColor: currentTheme.colors.border }}
						>
							<Button
								variant="ghost"
			...
			{deleteId && (
				...
				>
					...
					>
						<div
							className="p-6 bg-red-500/10 border-b flex items-start gap-4"
							style={{ borderColor: currentTheme.colors.border }}
						>
							...
							<div>
								...
								>
									{affectedUsers.length > 0
										? `Este perfil é usado por ${affectedUsers.length} usuários. Migre-os antes de excluir.`
						...
						<div className="p-6">
							{affectedUsers.length > 0 ? (
								<div className="space-y-6">
									<div
										className="p-4 rounded-lg border bg-blue-500/5"
										style={{
											borderColor:
												'rgba(59, 130, 246, 0.2)',
										}}
									...
									<div
										className="p-4 rounded-lg border bg-red-500/5"
										style={{
											borderColor:
												'rgba(239, 68, 68, 0.2)',
										}}
