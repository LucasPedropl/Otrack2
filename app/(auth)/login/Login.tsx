...
const LoginPage: React.FC = () => {
	...
	const handleEmailLogin = async (e: React.FormEvent) => {
		...
		try {
			...
			setError(
				err.message ||
					'Erro ao acessar o sistema. Verifique o email e senha.',
			);
		} finally {
			setIsLoading(false);
		}
				...
				<form onSubmit={handleEmailLogin} className="space-y-4">
					...
					<div className="flex items-center justify-between">
						<div className="flex items-center">
							...
							<label
								htmlFor="remember-me"
								className="ml-2 block text-sm select-none cursor-pointer"
								style={{ color: currentTheme.colors.text }}
							>
								Manter conectado
							</label>
					...
					{successMsg && (
						<div className="p-3 bg-green-500/10 text-green-600 text-sm rounded-lg border border-green-500/20">
							{successMsg}
						</div>
					)}
